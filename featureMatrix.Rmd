...
PAT.FEATMAT$quarter <- floor(as.numeric(PAT.FEATMAT$IXDAYS)/90)
...
PAT.FEATMAT <- 
  PAT.FEATMAT %>%
  select(ENROLID, IXDAYS, SUBSTANCENAME, MHSAFL, DIAG, HOSPFL, DAYSUPP, METQTY, AGE, REGION, SEX, quarter) %>%
  distinct()
```

## filter SUBSTANCES in PATIENTS-table by frequency
```{r}
important.subst <- DRUG.FEATMAT$SUBSTANCENAME

PAT.FEATMAT.S  <- 
  PAT.FEATMAT %>%  
  filter(SUBSTANCENAME %in% important.subst)
```

## filter DISEASES in PATIENTS-table by frequency
create PAT.FEATMAT.D
```{r}
important.diag  <- DISEASES.FEATMAT$ICD9

PAT.FEATMAT.D <- 
  PAT.FEATMAT %>%  
  filter(DIAG %in% important.diag)
```

## delete DIAG's / SUBSTANCENAME's in towrop with above, but with frequency < thres
```{r}
x <- PAT.FEATMAT.S %>% select(DIAG) %>% filter(!(DIAG %in% important.diag), !is.na(DIAG)) %>% distinct()
PAT.FEATMAT.S$DIAG[PAT.FEATMAT.S$DIAG %in% x$DIAG] <- NA

x <- PAT.FEATMAT.D %>% select(SUBSTANCENAME) %>% filter(!(SUBSTANCENAME %in% important.subst), SUBSTANCENAME != "") %>% distinct()
...
PAT.FEATMAT$SUBSTANCENAME <- as.character(PAT.FEATMAT$SUBSTANCENAME)
...
start <- proc.time()
q1$METQTY <- as.numeric(q1$METQTY)
...
# DISEASE
#########
PAT.FEATMAT.D.raw <- PAT.FEATMAT.D.raw %>% filter(DIAG %in% important.diag) %>% distinct()
PAT.FEATMAT.D.raw$DIAG <- as.numeric(PAT.FEATMAT.D.raw$DIAG)
...
for (n in length(PAT.FEATMAT.D.list)) {
  PAT.FEATMAT.D.list[[n]] <- 
    PAT.FEATMAT.D.list[[n]] %>% 
    left_join(DISEASES.FEATMAT, by = c("DIAG" = "ICD9")) %>% 
    select(-DIAG, - quarter) %>% 
    distinct()
...
# SUBSTANCES
############
PAT.FEATMAT.S.raw <- PAT.FEATMAT.S.raw %>% filter(SUBSTANCENAME %in% important.subst) %>% distinct()
...
to-do:
bevor es hier weitergeht muss das Problem betreffend der Grösse der Subpopulationen
in Abhängigkeit von den Filter- und Parsingoptionen geklärt werden!

Dann klären, wieso nur 31 von 47 möglichen Diagnosen in PAT.FEATMAT wiederzufinden sind!
Dann klären, wieso nur 4 von 115 möglichen HK's in PAT.FEATMAT wiederzufinden sind!
...
x <- aggregate(IXDAYS ~., PAT.MC.FLAG, min) %>% mutate(isIncident = IXDAYS >= 180) %>% distinct()
