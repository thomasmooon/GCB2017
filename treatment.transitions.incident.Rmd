...
children.incident <- FILTER_PATIENTS_AGE_REGION(TABLE04SAS7BDAT.sparse, sparse = T, cohort = "children")
...
children.incident.all.tm <-  TreatmentTransition(children.incident.all,title,nCutoff = nCutoff)
```
...
children.incident.mc.l[[m]] <- TreatmentTransition(children.incident.tmp, title, nCutoff = nCutoff)
}
```
...
adult.incident <- FILTER_PATIENTS_AGE_REGION(TABLE04SAS7BDAT.sparse, sparse = T, cohort = "adult")
...
adult.incident.all.tm <- TreatmentTransition(adult.incident.all,title,nCutoff = nCutoff)
```
...
for(m in 1:length(adult.incident.mc)) {
  ...
  adult.incident.mc.l[[m]] <- TreatmentTransition(adult.incident.tmp,title, nCutoff = nCutoff)
}
```
