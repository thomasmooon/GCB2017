...
subtitle: "children (AGE <= 16), patients with incident Main-CM only"
...
knitr::opts_chunk$set(echo = TRUE, fig.width=20, fig.height=15)
...
cat("Sum of incident patients: ", inc.unspec$sum.prevalent,"\n\n")
...
for (AED in AEDs[2:length(AEDs)]) {
  prev.vs.inc.byAED <- rbind(prev.vs.inc.byAED, IncPrevByAED(PATIENTS_AGE_REGION, AED))
}
```
...
ggplot(prev.vs.inc.byAED, aes(x = MAIN.COMORB, y=percent.incident)) + 
  ...
  geom_text(aes(label=paste(percent.incident*100,"% =",n.incident,"/",n.prevalent)), hjust=-0) +
  ylim(0,1.4) +
  coord_flip()
