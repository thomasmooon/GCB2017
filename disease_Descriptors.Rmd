...
### add diagnosis which are related to main-comorbidities
```{r}
DIAG.MC <-
  PATIENTS_AGE_REGION %>% 
  filter(!is.na(MAIN.COMORB), DIAG != "") %>%
  select(DIAG) %>%
  distinct()

relevant.diag <- unique(c(relevant.diag$DIAG, DIAG.MC$DIAG))
```
...
ICD9GROUPING <- ICD9GROUPING %>% filter(ICD9 %in% relevant.diag)
```
...
# filter by relevant diagnosis
MESH2ICD9 <- MESH2ICD9 %>% filter(icd9 %in% as.numeric(relevant.diag))
```
...
# filter by relevant diagnosis
DISEASE2BIOMARKER <- DISEASE2BIOMARKER %>% filter(ICD9 %in% relevant.diag)
...
# filter by relevant diagnosis
DISEASE2SYMPTOMS <- DISEASE2SYMPTOMS %>% filter(ICD9 %in% relevant.diag)
```
...
# filter by relevant diagnosis
disease2genes <- disease2genes %>% filter(ICD9 %in% relevant.diag)
...
```{r}
DISEASES          <- relevant.diag %>% dplyr::rename(ICD9 = DIAG)
