mRMR.Ranger2 <-
  function(train,
    ...
    featureSelectionMRMR2 <- function(featureMatrix, feature_count = 500){
      ...
      fs <- data.frame(scores = unlist(fs@scores), featureNames) %>% filter(scores > 0)
      return(fs)
    }
    ...
    StatusEmail(subject = paste("RStudio status email:",", model =",modelDescription,", MC = ",mc,", CV:" ,k,", ncores: ",ncores,", min.node.size: ", min.node.size,"training time minutes",", # features = ",length(featureSelection$featureNames)),body = body) 
