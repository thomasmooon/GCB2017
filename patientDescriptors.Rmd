...
P.AED_QUARTQTY <- GetQUARTQTY(P)
  ...
  # drop features which are not in medical history
  source("functions/NoMedicalHistoryColumns.R")
  (noMHCols <- NoMedicalHistoryColumns(P.AED_QUARTQTY))
  (noMHCols <- setdiff(noMHCols,"ENROLID"))
  colSelect <- setdiff(colnames(P.AED_QUARTQTY), noMHCols)
  P.AED_QUARTQTY <- P.AED_QUARTQTY[,colSelect]
