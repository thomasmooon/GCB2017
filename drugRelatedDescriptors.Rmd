...
only for frequent substances drug features will be computed
```{r start, echo=FALSE}
P                   <- FILTER_PATIENTS_AGE_REGION(TABLE04SAS7BDAT)
subst.freq          <- SUBSTANCE_FREQUENCY(P)
relevant.subst      <- subst.freq %>% filter(freq >= params$thres.subst.freq) %>% dplyr::select(SUBSTANCENAME)

PATIENTS.DRUGS <- 
  P %>% filter(SUBSTANCENAME %in% relevant.subst$SUBSTANCENAME) %>% 
