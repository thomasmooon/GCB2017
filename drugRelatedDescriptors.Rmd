...
```{r}
drug2indications1 <- inner_join(DRD2, DRUG2INDICATIONS2, by=c("SUBSTANCENAME"="DRUG_DESC"))
```
...
DRUG2SIDEEFFECTS <- 
  rbind(
    DRUG2SIDEEFFECTS %>% select(-STITCHID_STEREO) %>% dplyr::rename(STITCHID = STITCHID_FLAT ),
    DRUG2SIDEEFFECTS %>% select(-STITCHID_FLAT) %>% dplyr::rename(STITCHID = STITCHID_STEREO )
  ) %>% distinct()
...
DRUGIDS <- 
  ...
  select(FLAT_CHEMICAL, STEREO_CHEMICAL, SOURCE_ID) %>% 
  distinct() %>% collect() %>% 
...
```{r}
drug2sideeffects <- inner_join(drug2sideeffects2, DRUG2SIDEEFFECTS3, by="ATC")
drug2sideeffects <- NormalizeStrings(drug2sideeffects)
```
...
PATIENTS.DRUGS <- NormalizeStrings(PATIENTS.DRUGS)
```
