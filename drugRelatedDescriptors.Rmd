...
u2g <- select(org.Hs.eg.db, keys=u, keytype="UNIPROT" ,columns=c("UNIPROT","GO")) %>% dplyr::select(UNIPROT, GO) %>% distinct()
...
uniprot2kegg <- 
  select(org.Hs.eg.db, keys=u, keytype="UNIPROT" ,columns=c("UNIPROT","PATH")) %>% 
...
u <- unique(drug2uniprot$UNIPROT.ID)
uniprot2entrez <- 
  select(org.Hs.eg.db, keys=u, keytype="UNIPROT" ,columns=c("UNIPROT","ENTREZID")) %>%
...
# # replace double dot with dot
# ReplaceDoubleDot <- function(x)  gsub("\\.{2,}",".",x)
# Wrap.ReplaceDoubleDot <- function(x) apply(x,2,function(xx) ReplaceDoubleDot(xx)) %>% as_tibble() %>% distinct()
# 
# fo.DRD1 <- Wrap.ReplaceDoubleDot(fo.DRD1)
# fo.drug2indications.1 <- Wrap.ReplaceDoubleDot(fo.drug2indications.1)
# fo.drug2indications.2 <- Wrap.ReplaceDoubleDot(fo.drug2indications.2)
# fo.drug2sideeffects <- Wrap.ReplaceDoubleDot(fo.drug2sideeffects)
# fo.drug2go <- Wrap.ReplaceDoubleDot(fo.drug2go)
# fo.drug2pathway <- Wrap.ReplaceDoubleDot(fo.drug2pathway)
