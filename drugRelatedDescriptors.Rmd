...
load("output/SUBST.PROP.PHARM.RData")
```

# retrieve SUBSTANCENAME, PROPRIETARYNAME, PHARMCLS1
```{r}
load("output/SUBST.PROP.PHARM.RData")
SUBST.PROP.PHARM <- SUBST.PROP.PHARM %>% filter(SUBSTANCENAME != "") %>%  distinct()
PATIENTS.DRUGS1 <- SUBST.PROP.PHARM
```
...
# spread features for joins
  ...
  fo.DRD1 <- fo.DRD1 %>% dplyr::rename(DRUG.PHARMCLS1 = PHARMCLS1)
  ...
  ## spread
  fo.DRD1               <- spread(data.frame(fo.DRD1,key=1), DRUG.PHARMCLS1, key, fill = 0, sep = ".x.")
...
# print summary
tables <- c("drug2pharmcls1","drug2phewas_hl","drug2phewas", "drug2go", "drug2pathway (kegg & wiki)", "drug2sideeffects","drug2tissue")
dims   <- c(paste(dim(fo.DRD1),collapse=" x "),
