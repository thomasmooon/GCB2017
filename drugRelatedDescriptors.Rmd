...
```{r, include = FALSE}
rm(list=ls())
knitr::opts_chunk$set(echo = TRUE, fig.width=10, fig.height=7) # markdown
source("z_start.R") # initalize netezza DB, load core data
```

# retrieve synonyms and classes for substancenames
```{r}
load("~/epilepsie_preprocessing_descrStat/data/re-do_epd183/pharm_class_mapping.RData")

pharmClassMapping <- 
  pharmClassMapping %>% filter(GENNME != "", THRCLDS != "") %>%  
  select(GENNME, THRCLDS, THRGRDS) %>% distinct() %>% rename(SUBSTANCENAME = GENNME)

pharmClassMapping <- apply(pharmClassMapping, 2, function(x) toupper(trimws(x))) %>% as.data.frame() %>% unique()
```

# add Synonyms
```{r}
DRUG2SYNONYMS <- tbl(src,"REFERENCE.DRUGSYNONYMS") %>% filter(TTDDRUGID != "") %>% collect() 
...
PATIENTS.DRUGS2 <-left_join(pharmClassMapping,DRUG2SYNONYMS %>% dplyr::select(-TTDDRUGID) %>% distinct(), by = c("SUBSTANCENAME" = "DRUGNAME"))
```
...
PATIENTS.DRUGS3 <- 
  rbind(
    PATIENTS.DRUGS2 %>% dplyr::select(SUBSTANCENAME,THRCLDS,THRGRDS) %>% mutate(names=SUBSTANCENAME),
    PATIENTS.DRUGS2 %>% dplyr::select(SUBSTANCENAME,THRCLDS,THRGRDS,SYNONYMS) %>% dplyr::rename(names=SYNONYMS)
  ) %>% 
  distinct()
...
DrugSynonymsDrugBank$Synonyms <- as.character(DrugSynonymsDrugBank$Synonyms)
...
* 2017-FEB-02
```{r}
load("data/uniprot2tissue.RData")
drug2tissue <- inner_join(drug2uniprot, uniprot2tissue, by = c("UNIPROT.ID" = "UNIPROT")) %>% dplyr::select(TTDDRUGID, tissue) %>% distinct()
```
...
# keep only BP class
cat("\n keep only BP-class GO terms \n")
pattern <- "^BP"
BP <- grep(pattern, drug2go$GO)
drug2go <- drug2go[BP,]
```
...
```{r}
fi.drugclass <- DRD1 %>% dplyr::select(-names) %>% distinct()
...
fo.THRCLDS            <- fi.drugclass %>% dplyr::select(SUBSTANCENAME, THRCLDS) %>% distinct()
fo.THRGRDS            <- fi.drugclass %>% dplyr::select(SUBSTANCENAME, THRGRDS) %>% distinct()
...
fo.drug2tissue        <- AddSubstancenames(fi.drug2tissue)

# make.names()
My.make.names <- function(x) apply(x, 2, make.names) %>% as_tibble() %>% distinct()
fo.THRCLDS          <- My.make.names(fo.THRCLDS)
fo.THRGRDS          <- My.make.names(fo.THRGRDS)
...
fo.drug2tissue      <- My.make.names(fo.drug2tissue)
save(fo.drug2pathway,file="output/fo.drug2pathway.RData")

# stats for fo's
FeatStats(fo.THRCLDS)
FeatStats(fo.THRGRDS)
...
FeatStats(fo.drug2tissue)

# spread features for joins
  ## rename for spread to retrieve meaningful variable names
  fo.THRCLDS <- fo.THRCLDS %>% dplyr::rename(DRUG.THRCLDS = THRCLDS)
  fo.THRGRDS <- fo.THRGRDS %>% dplyr::rename(DRUG.THRGRDS = THRGRDS)
  fo.drug2indications.1 <- fo.drug2indications.1 %>% dplyr::rename(DRUG.PHEWAS_HL = PHEWAS_STRING_HL)
  fo.drug2indications.2 <- fo.drug2indications.2 %>% dplyr::rename(DRUG.PHEWAS = PHEWAS_STRING)
  fo.drug2go <- fo.drug2go %>% dplyr::rename(DRUG.GO = GO)
  fo.drug2pathway <- fo.drug2pathway %>% dplyr::rename(DRUG.PATHWAY = PATHWAY)
  fo.drug2tissue <- fo.drug2tissue %>% dplyr::rename(DRUG.TISSUE = tissue)
  fo.drug2sideeffects <- fo.drug2sideeffects %>% dplyr::rename(DRUG.SIDEEFFECTS = SIDEEFFECTS)
  ## spread
  fo.THRCLDS            <- spread(data.frame(fo.THRCLDS,key=1), DRUG.THRCLDS, key, fill = 0, sep = ".x.")
  fo.THRGRDS            <- spread(data.frame(fo.THRGRDS,key=1), DRUG.THRGRDS, key, fill = 0, sep = ".x.")
  fo.drug2indications.1 <- spread(data.frame(fo.drug2indications.1,key=1), DRUG.PHEWAS_HL, key, fill = 0, sep = ".x.")
  fo.drug2indications.2 <- spread(data.frame(fo.drug2indications.2,key=1), DRUG.PHEWAS, key, fill = 0, sep = ".x.")
  fo.drug2go            <- spread(data.frame(fo.drug2go,key=1), DRUG.GO, key, fill = 0, sep = ".x.")
  fo.drug2pathway       <- spread(data.frame(fo.drug2pathway,key=1), DRUG.PATHWAY, key, fill = 0, sep = ".x.")
  fo.drug2tissue        <- spread(data.frame(fo.drug2tissue,key=1), DRUG.TISSUE, key, fill = 0, sep = ".x.")
  fo.drug2sideeffects <- unite(fo.drug2sideeffects, DRUG2SIDEEFFECTxFREQ, DRUG.SIDEEFFECTS, WHO, sep=".x.")
  fo.drug2sideeffects <- spread(data.frame(fo.drug2sideeffects,key=1), DRUG2SIDEEFFECTxFREQ, key, fill = 0, sep = ".x.")
```
...
# print summary
tables <- c("drug2THRCLDS","fo.THRGRDS","drug2phewas_hl","drug2phewas", "drug2go", "drug2pathway (kegg & wiki)", "drug2sideeffects","drug2tissue")
dims   <- c(paste(dim(fo.THRCLDS),collapse=" x "),
            paste(dim(fo.THRGRDS),collapse=" x "),
            paste(dim(fo.drug2indications.1),collapse=" x "),
            paste(dim(fo.drug2indications.2),collapse=" x "),
            paste(dim(fo.drug2go),collapse=" x "),
            paste(dim(fo.drug2pathway),collapse=" x "),
            paste(dim(fo.drug2sideeffects),collapse=" x "),
            paste(dim(fo.drug2tissue),collapse=" x ")
            )

s           <- as.data.frame(dims,tables)
colnames(s) <- "nSUBSTANCENAMES x nFeatures"
knitr::kable(s)

# combine
drug.features <-
  fo.THRCLDS %>% 
  left_join(fo.THRGRDS) %>% 
  ...
  left_join(fo.drug2tissue)

# NA substitution
drug.features[is.na(drug.features)] <- 0
