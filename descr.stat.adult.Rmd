...
```{r}
main.comorb.first.ixday <- PATIENTS_AGE_REGION %>% filter(MAIN.COMORB != "") %>% select(ENROLID, IXDAYS, MAIN.COMORB) %>% distinct()
...
x <- main.comorb.first.ixday %>% filter(IXDAYS >= params$incidence.buffer) %>% select(ENROLID) %>% distinct()
...
for (m in MAIN.COMORB) {
  patObsCount$addCount(
    PATIENTS_AGE_REGION %>% filter(ENROLID %in% x$ENROLID, MAIN.COMORB == m),
    paste("-",m)
    )  
}
...
### by MAIN.COMORB
```{r, echo = FALSE}
knitr::kable(MAIN.COMORB.PHEWAS %>% group_by(MAIN.COMORB) %>% select(n) %>% tally())
```
...
# plot patient frequency by filter criterium
```{r}
