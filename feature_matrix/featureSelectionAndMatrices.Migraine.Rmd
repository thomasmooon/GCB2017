...
library(ranger)
library(survival)
profile.train <- profvis::profvis(x.ranger <- ranger(Surv(time, status) ~ ., data = mc.Features$fm.training, num.trees = 5000)) # train
profile.test <- profvis::profvis(x.predict <- predict(x.ranger, dat = mc.Features$fm.test$featureMatrix)) # predict
```
...
# status email
```{r}
stop <- round((proc.time() - start)[3] / 60,0)
source("functions/StatusEmail.R")
StatusEmail(subject = paste("RStudio status email: ",mc.descr,"finished"),body = paste(stop,"minutes computing time"))
```
